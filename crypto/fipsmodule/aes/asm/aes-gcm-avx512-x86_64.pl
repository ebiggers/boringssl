...
sub _ghash_square {
    ...
    return <<___;
    vpclmulqdq      \$0x00, $a, $a, $t0        # LO = a_L * b_L
    vpclmulqdq      \$0x11, $a, $a, $dst       # HI = a_H * b_H
    vpclmulqdq      \$0x01, $t0, $gfpoly, $t1  # LO_L*(x^63 + x^62 + x^57)
    vpshufd         \$0x4e, $t0, $t0           # Swap halves of LO
    vpxor           $t1, $t0, $t0              # Fold LO into MI
    vpclmulqdq      \$0x01, $t0, $gfpoly, $t1  # MI_L*(x^63 + x^62 + x^57)
    vpshufd         \$0x4e, $t0, $t0           # Swap halves of MI
    vpternlogd      \$0x96, $t1, $t0, $dst     # Fold MI into HI
___
}
